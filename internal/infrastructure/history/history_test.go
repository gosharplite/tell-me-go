...
func FuzzManager_RollbackTurns(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, turns int) {
		runRollbackSimulation(t, turns)
	})
}

func runRollbackSimulation(t *testing.T, turns int) {
	...
	for _, hasSystem := range []bool{false, true} {
		for initialLen := 9; initialLen <= 10; initialLen++ {
			m := setupRollbackFuzzState(initialLen, hasSystem)
			ctx := context.Background()
			...
			validateRollbackInvariants(t, m, turns, initialLen, hasSystem, actualRemoved, remainingTurns, remainingMsgs)
		}
	}
}

func setupRollbackFuzzState(initialLen int, hasSystem bool) *Manager {
	m := &Manager{
...
func validateRollbackInvariants(t *testing.T, m *Manager, turns, initialLen int, hasSystem bool, actualRemoved, remainingTurns, remainingMsgs int) {
	t.Helper()
	finalLen := len(m.Contents)
	validateBoundsInvariants(t, finalLen, initialLen, actualRemoved, turns)
	validateStructureInvariants(t, finalLen, hasSystem, turns, remainingTurns, remainingMsgs)
}

func validateBoundsInvariants(t *testing.T, finalLen, initialLen, actualRemoved, turns int) {
	if finalLen < 0 {
...
func validateStructureInvariants(t *testing.T, finalLen int, hasSystem bool, turns, remainingTurns, remainingMsgs int) {
	expectedParity := 0
