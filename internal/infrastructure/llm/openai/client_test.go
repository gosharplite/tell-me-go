...
func TestToOpenAIMessages_EmptyContent(t *testing.T) {
	...
	messages, _ := c.toOpenAIMessages(context.Background(), history, nil, false)
	if len(messages) != 2 {
...
func TestDeepSeekHistoryWithToolCalls(t *testing.T) {
	...
	messages, _ := client.toOpenAIMessages(context.Background(), history, nil, false)
	if len(messages) != 2 {
		t.Fatalf("expected 2 messages, got %d", len(messages))
	}
...
func TestInjectPersona(t *testing.T) {
	t.Run("OpenAI Reasoner Persona", func(t *testing.T) {
		...
		messages, _ := c.toOpenAIMessages(context.Background(), []*llm.Content{{Role: "user", Parts: []*llm.Part{{Text: "Hi"}}}}, nil, false)
...
func TestToOpenAIMessages_MultiToolResponse(t *testing.T) {
	...
	messages, err := c.toOpenAIMessages(context.Background(), history, nil, false)
	if err != nil {
