...
func TestSendChat(t *testing.T) {
	...
	client, err := NewClient(apiURL, "test-model", authenticator, 0, "", 0, "", false, events.NewSimpleEventBus(), 5*time.Second)
	if err != nil {
...
func TestSystemInstruction(t *testing.T) {
	...
	apiURL := server.URL + "/aiplatform.googleapis.com/v1/projects/p/locations/l/publishers/google/models"
	client, _ := NewClient(apiURL, "test-model", &auth.VertexAuth{Token: "test"}, 0, "", 0, expectedInstruction, false, events.NewSimpleEventBus(), 5*time.Second)
...
func TestThinkingBudget(t *testing.T) {
	...
	apiURL := server.URL + "/aiplatform.googleapis.com/v1/projects/p/locations/l/publishers/google/models"
	client, _ := NewClient(apiURL, "test-model", &auth.VertexAuth{Token: "test"}, 1000, "LOW", 0, "", false, events.NewSimpleEventBus(), 5*time.Second)
