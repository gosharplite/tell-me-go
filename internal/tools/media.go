...
func RegisterMediaTools(r *Registry, client *api.Client) {
	...
	r.Register(&genai.FunctionDeclaration{
		Name:        "create_video",
		Description: "Generates a short video from a text prompt using Veo. Saves to assets/generated/.",
		Parameters: &genai.Schema{
			Type: genai.TypeObject,
			Properties: map[string]*genai.Schema{
				"prompt": {
		...
		return createVideo(args, client)
	})
}

func createVideo(args map[string]interface{}, client *api.Client) (string, error) {
	prompt, _ := args["prompt"].(string)

	fmt.Fprintf(os.Stderr, "\033[0;36m[Tool Action] Generating video: %s (This may take several minutes...)\033[0m\n", prompt)

	// Large timeout for video generation
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Minute)
	defer cancel()

	// Use Veo 2.0
	// Note: GCS output might be needed if using Vertex AI.
	videos, err := client.GenerateVideos(ctx, "veo-2.0-generate-001", prompt, "")
	if err != nil {
		return fmt.Sprintf("Error generating video: %v", err), nil
	}

	if len(videos) == 0 {
		return "Error: No videos were generated.", nil
	}
	...
	filename := fmt.Sprintf("%s_%s.mp4", timestamp, safePrompt)
	outPath := filepath.Join(outDir, filename)

	if err := os.WriteFile(outPath, videos[0], 0644); err != nil {
		return fmt.Sprintf("Error saving video: %v", err), nil
	}

	return fmt.Sprintf("Video generated successfully and saved to: %s", outPath), nil
}
