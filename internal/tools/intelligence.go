...
// RegisterIntelligenceTools adds AST-based tools to the registry by delegating to specialized managers.
func RegisterIntelligenceTools(r *Registry, sm *SecurityManager) {
	nav := &navigation.Manager{SP: sm}
	ref := &refactor.Manager{SP: sm}
	ana := &analysis.Manager{SP: sm}
	inf := &info.Manager{SP: sm}
	sea := &search.Manager{SP: sm}
	...
	}, ref.RenameSymbol, ToolOptions{Serial: true, LongRunning: true})
	...
	}, ref.MoveDefinition, ToolOptions{Serial: true})
}
