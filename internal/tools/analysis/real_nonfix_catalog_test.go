...
func TestVerifyNonFixCatalog(t *testing.T) {
	...
	expectedCataloged := map[string]funcComplexity{
		...
		"(*plurHook).AfterTurn":                                     {Line: 97, Complexity: 22, FilePath: "internal/agent/memory/hook.go"},
		"(*plurHook).FlushSession":                                  {Line: 300, Complexity: 18, FilePath: "internal/agent/memory/hook.go"},
...
func TestVerifyCoveragePinsMatchLiveCatalog(t *testing.T) {
	...
	tests := []struct {
		...
		{"hook.go FlushSession engrams-empty belt-and-suspenders", "internal/agent/memory/hook.go", 361, 363},
