...
// TestIsInTargetScope_AbsFails verifies that when filepath.Abs fails
// (e.g. due to a null byte in the path), isInTargetScope falls back
// to the raw directory path instead of silently returning false.
func TestIsInTargetScope_AbsFails(t *testing.T) {
	t.Parallel()
	...
	// Should not panic; should return true because the fallback path
	// "/some/valid/prefix" matches the targetPath.
	assert.True(t, analyzer.isInTargetScope(pkg, state),
		"isInTargetScope should return true when Abs fails but dirname matches targetPath")
}
