...
func TestGitTools(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			reg := registry.New()
			if err := Register(reg, sm, executor, security.NewCommandValidator(sm, nil), persistencetest.NewPlainOSFileSystem(), infra_persistence.NewWorkspacePolicy(), nil, nil, &toolstest.FakeProcessRunner{}); err != nil {
