...
func TestToolExecutor_ConcurrentExecution(t *testing.T) {
	...
	toolFn := func(ctx context.Context, args map[string]interface{}) (tools.ToolResult, error) {
		mu.Lock()
		activeCount++
	...
	reg.Register(&tools.ToolDeclaration{Name: "t1"}, toolFn)
	reg.Register(&tools.ToolDeclaration{Name: "t2"}, toolFn)
	reg.Register(&tools.ToolDeclaration{Name: "t3"}, toolFn)
...
func TestToolExecutor_SerialExecution(t *testing.T) {
	...
	toolFn := func(name string) func(context.Context, map[string]interface{}) (tools.ToolResult, error) {
	...
	reg.Register(&tools.ToolDeclaration{Name: "p1"}, toolFn("p1"))
	reg.RegisterWithOptions(&tools.ToolDeclaration{Name: "s1"}, toolFn("s1"), registry.ToolOptions{Serial: true})
	reg.Register(&tools.ToolDeclaration{Name: "p2"}, toolFn("p2"))
...
func TestToolExecutor_Timeout(t *testing.T) {
	...
	reg.Register(&tools.ToolDeclaration{Name: "slow"}, func(ctx context.Context, args map[string]interface{}) (tools.ToolResult, error) {
		select {
		case <-ctx.Done():
