...
func TestSessionManager_ApplyConfiguration_Error(t *testing.T) {
	...
	deps := newSessionDependencies(paths, nil, nil, nil, nil, nil, nil, pData, nil, nil, slog.Default(), nil, new(mockSessionProvider))
...
func TestFormatSystemMessageForLog(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := formatSystemMessageForLog(tt.msg, tt.level, now)
