...
func TestValidateMetrics(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "plausible throughput",
			metrics: &llm.Metrics{
				ResponseTokens: 100,
				Duration:       2.0, // 50 tokens/sec
			},
			provider:      "openai",
			model:         "gpt-4",
			expectWarning: false,
		...
		{
			name: "implausible throughput triggers warning",
			metrics: &llm.Metrics{
				ResponseTokens: 5000,
				Duration:       0.1, // 50,000 tokens/sec
			},
			provider:      "anthropic",
			model:         "claude-3-opus",
			expectWarning: true,
		...
		{
			name: "exactly at threshold (5000 tokens/sec)",
			metrics: &llm.Metrics{
				ResponseTokens: 5000,
				Duration:       1.0,
			},
		...
		{
			name: "just above threshold (5001 tokens/sec)",
			metrics: &llm.Metrics{
				ResponseTokens: 5001,
				Duration:       1.0,
			},
		...
		{
			name: "different provider/model combination",
			metrics: &llm.Metrics{
				ResponseTokens: 10000,
				Duration:       0.5, // 20,000 tokens/sec
			},
			provider:      "google",
			model:         "gemini-2.0",
			expectWarning: true,
		...
		{
			name: "cached tokens non-zero",
			metrics: &llm.Metrics{
				ResponseTokens: 10000,
				Duration:       0.1, // 100,000 tokens/sec
				CachedTokens:   50,
			},
			provider:      "openai",
			model:         "gpt-4",
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create inferenceStep with mocked logger
			step := &inferenceStep{}
			turn := &turn{
			...
			step.validateMetrics(tt.metrics, turn)

			// Verify warnings
			logOutput := logBuf.String()
			hasWarning := strings.Contains(logOutput, "implausible_token_throughput")

			if tt.expectWarning != hasWarning {
				t.Errorf("%s: expected warning %v, got %v (logs: %q)",
					tt.name, tt.expectWarning, hasWarning, logOutput)
			}

			// Additional assertions for warning fields
			if tt.expectWarning {
				// Verify specific fields in log output
				if !strings.Contains(logOutput, tt.provider) {
					t.Errorf("%s: warning missing provider field %q", tt.name, tt.provider)
				}
				if !strings.Contains(logOutput, tt.model) {
					t.Errorf("%s: warning missing model field %q", tt.name, tt.model)
				}
				// Check tokens_per_sec > 5000 is mentioned
				if !strings.Contains(logOutput, "tokens_per_sec") {
					t.Errorf("%s: warning missing tokens_per_sec field", tt.name)
				}
				// Ensure cached_tokens field appears (zero value)
				if !strings.Contains(logOutput, "cached_tokens") {
					t.Errorf("%s: warning missing cached_tokens field", tt.name)
				}
				// If cached tokens non-zero, ensure the value appears in log
				if tt.metrics != nil && tt.metrics.CachedTokens != 0 {
					if !strings.Contains(logOutput, "50") {
						t.Errorf("%s: warning missing cached_tokens value 50", tt.name)
					}
				}
			}
		})
