...
func TestStartSpinner_Synchronization(t *testing.T) {
	combined := inframock.NewSafeBuffer()
	locker := &mockLocker{}
	mc := &mockClock{now: time.Date(2026, 1, 1, 12, 0, 0, 0, time.UTC)}
...
func TestStdUIRenderer_SpinnerWithMetrics(t *testing.T) {
	stderr := inframock.NewSafeBuffer()
	locker := &mockLocker{}
	mc := &mockClock{now: time.Date(2026, 1, 1, 12, 0, 0, 0, time.UTC)}
...
func TestStdUIRenderer_TurnsLogWriter(t *testing.T) {
	stdout, stderr := inframock.NewSafeBuffer(), inframock.NewSafeBuffer()
	turnsLog := inframock.NewSafeBuffer()
	locker := &mockLocker{}
	mc := &mockClock{now: time.Date(2026, 1, 1, 12, 0, 0, 0, time.UTC)}
	r := NewRenderer(locker, stdout, stderr, mc, nil).(*stdUIRenderer)
	r.SetTurnsLogWriter(turnsLog)

	t.Run("Full Turn Cycle in TurnsLog", func(t *testing.T) {
		turnsLog.Reset()
		...
		// 1. Header & Pre-call tokens
		r.LogTurnStatus(context.Background(), status)
		
		// 2. Post-call metrics
		status.IsPostCall = true
		status.Metrics = &llm.Metrics{
			PromptTokens:   120,
			CachedTokens:   50,
			ResponseTokens: 80,
			ThinkingTokens: 40,
		...
		// 3. Final summary
		status.IsFinal = true
		status.TaskCost = 0.005
		status.SessionCost = 0.01
		status.DailyCost = 0.05
		status.TotalM = 100
		status.TotalH = 50
		status.TotalO = 150
		r.LogTurnStatus(context.Background(), status)
		...
		expectedLines := []string{
			"────────────────────────────────────────────────────────────────────────────────",
			"╭─⠿ Turn 1/5 - coder",
			"[12:00:00] Payload: ~100/1000 tokens - coder",
			"[12:00:00] Payload: 120/1000 tokens - coder",
			"[12:00:00] M: 70 H: 50 C: 80 Th: 40  ($0.0050) [2.50s (ΣT: 0.00s) / 0.00s (0.00)]",
			"╰─⠿ Ready ($0.0050 $0.0050 $0.0100 $0.0500 M: 100 H: 50 33.3% O: 150)",
		}

		for _, line := range expectedLines {
			if !strings.Contains(output, line) {
				t.Errorf("expected turns log to contain %q, got %q", line, output)
			}
		}
	})
}
