...
func TestExtractResponseText(t *testing.T) {
	t.Run("thought fallback when no visible text", func(t *testing.T) {
		...
		got := extractResponseText(content)
	...
	t.Run("visible text preferred over thought", func(t *testing.T) {
		...
		assert.Equal(t, "visible", got,
			"thought fallback must NOT trigger when visible text exists")
	})

	t.Run("nil content", func(t *testing.T) {
		got := extractResponseText(nil)
		assert.Equal(t, "", got, "nil content should return empty string")
	})
	...
	t.Run("multiple thoughts only — all concatenated", func(t *testing.T) {
		content := &llm.Content{
			Parts: []*llm.Part{
		...
		got := extractResponseText(content)
		assert.Equal(t, "thinking step 1thinking step 2", got)
	})

	t.Run("visible text with empty thought — thought ignored", func(t *testing.T) {
		content := &llm.Content{
			Parts: []*llm.Part{
				{Text: "visible", IsThought: false},
				{Text: "", IsThought: true},
