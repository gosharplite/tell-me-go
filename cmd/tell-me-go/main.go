...
func main() {
	...
	// 2. Load Config (needed for history path)
	cfg, err := config.Load(*configPath)
	if err != nil {
		log.Fatalf("Error loading config [%s]: %v", *configPath, err)
	}

	// 3. Initialize History Path
	...
	if *lastN > 0 {
		showHistory(hManager, *lastN)
	}

	// 4. Handle Prompt Argument and Stdin
	prompt := flag.Arg(0)
	...
	// 8. Prune and Save using MaxHistoryTurns
...
func showHistory(hManager *history.Manager, n int) {
