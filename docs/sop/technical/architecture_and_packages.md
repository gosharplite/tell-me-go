...
- **Tools-Layer Filesystem Boundary**: Tools-layer packages must not import `internal/infrastructure/persistence` — no exceptions — enforced by the `verify-tools-adapter-import` gate (ADR-055). The former `defaultFS` fallback pattern is fully retired (workspace by ADR-074 / issue #1460; analysis by issue #1465); every live tool path uses the injected domain port (`persistence.FileSystem`), and adapters are constructed only at the composition root (`internal/infrastructure/di`).
