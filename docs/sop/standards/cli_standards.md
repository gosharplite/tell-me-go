...
- Defaults should be sensible (e.g., default config path to `configs/butler.yaml`).
...
func (c *chatCommand) parseFlags(args []string) (*cliOptions, *flag.FlagSet, error) {
	...
	opts := &cliOptions{}
	fs.StringVar(&opts.configPath, "c", "configs/butler.yaml", "Path to config")
...
- **Completeness**: All configuration templates (e.g., `configs/butler.yaml`) **MUST** include every variable supported by the application's `Config` struct.
