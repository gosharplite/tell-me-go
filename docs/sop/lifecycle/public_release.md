...
6.  **Confirm Target Version**: Use `git tag -l` to check the last release version. Use `ask_user` to present the last version and propose the target release version (e.g., `5.6.0`). Store this in the `manage_scratchpad`.
...
3.  **Build and Verify Binary**: 
    ...
    ```bash
    go build -ldflags="-X 'main.version=5.6.0'" -o tell-me-go ./cmd/tell-me-go
    # OR
    make build VERSION=5.6.0
    ```
