...
- **See**: `internal/tools/workspace/process_executor_stream_test.go:818` (re-anchored 2026-09 (#1460 T6): 817→818 as the T5 comment-hygiene edit grew TestRunPipeline_NewPipelineError's doc above it; CC re-verified 11 — unchanged)
...
### internal/tools/toolstest/fake_process_runner_test.go — TestFakeProcessRunner_PresetFunc (CC=12)

- **Status**: ACCEPTED (2026-09, issue #1460)
- **Rationale**: Table-driven test with 2 subtests (preset handle passthrough, error passthrough) asserting StartFunc returns, call-log recording, and last-Call identity via the assertStartLog helper. CC=12 comes from subtest enumeration and assertion boilerplate, not branching business logic — same acceptance class as `TestFakeToolchainRunner_PresetValues` (CC=28). Grew over the CC=10 threshold at introduction (T2) because the handle subtest also pins the `tools.ExitError` round-trip (errors.As + the "exit status N" wording); flagged by the partition gate at T4.
- **See**: `internal/tools/toolstest/fake_process_runner_test.go:52`

### internal/tools/workspace/process_executor_fault_test.go — TestRunCommandFaultPaths (CC=26), TestRunPipelineFaultCleanup (CC=16)

- **Status**: ACCEPTED (2026-09, issue #1460)
- **Rationale**: The fake-based fault suites introduced by T6. `TestRunCommandFaultPaths` (CC=26) is a 7-subtest table (start-error wrapping, exit-code surfacing, −1 signal convention, non-ExitError passthrough, ctx-cancel partial output, erroring-reader warning, happy path) with per-case `verify` closures — CC comes from subtest enumeration and per-case assertion boilerplate, not branching business logic. `TestRunPipelineFaultCleanup` (CC=16) covers pipeline start-error cleanup across two failing-stage indices plus the arity and spec-assembly guards with call-log assertions. Same acceptance class as `TestFakeToolchainRunner_PresetValues` (CC=28) — test-complexity.
- **See**: `internal/tools/workspace/process_executor_fault_test.go:29,280`

### internal/tools/workspace/pipeline_fault_test.go — TestPipelineWaitFaultSemantics (CC=15)

- **Status**: ACCEPTED (2026-09, issue #1460)
- **Rationale**: Five-subtest wait-semantics pin over three stages with per-stage fake handles (last-stage exit code, non-last-stage-only exit code with clean lastErr, last-stage non-ExitError propagation, mixed-failure reverse-order exit code, erroring-stderr capture warning) driven through a shared keyed-StartFunc helper. CC comes from subtest enumeration and per-case assertion boilerplate, not branching business logic — same test-complexity class.
- **See**: `internal/tools/workspace/pipeline_fault_test.go:23`

### internal/tools/workspace/shell_fault_test.go — TestShellFaultPipeCommands (CC=14), TestShellFaultExecuteCommand (CC=12)

- **Status**: ACCEPTED (2026-09, issue #1460)
- **Rationale**: Two-layer shell fault suites (fake runner → real processExecutor → real shellTool). `TestShellFaultPipeCommands` (CC=14) covers pipeline result formatting across success, start-error, and deadline subtests plus call-order-keyed stage handles; `TestShellFaultExecuteCommand` (CC=12) covers the start-error ToolResult, exit-code formatting, and timeout formatting with partial output. CC comes from subtest enumeration and assertion boilerplate, not branching business logic — same test-complexity class.
- **See**: `internal/tools/workspace/shell_fault_test.go:108,35`
...
### di/toolchain_factory.go — registerSkillsShTools error (structurally unreachable)

- **Status**: ACCEPTED (2026-09)
- **Rationale**: the branch requires `skillssh.RegisterSkillsShTools` to fail, which happens only for a nil registry ("registry is required") or an empty tool name (`validateRegistration`) — `BuildRegistry` supplies `registry.New()` and four fixed non-empty names, so the branch cannot fire in correct code; repo-fetch failures never propagate (skillssh treats them as best-effort strings). Re-classified 2026-09 (#1462) from the earlier filesystem fault-injection rationale after live-tree fact-check. Re-anchored 2026-09 (#1431): lines drifted 124-126 → 127-129 as the skillssh registration call-site comment block grew the file above the branch. Re-anchored 2026-09 (#1462): lines drifted 127-129 → 132-134 as the #1460 T4 process-runner construction block grew the file above the branch; re-verified against the live source.
- **See**: `internal/infrastructure/di/toolchain_factory.go:132-134`
...
- **Rationale**: nil/empty guards on internal state that callers never produce —
  ...
  Re-anchored 2026-09 (#1460 T4): the executor's defensive empty-parts guard
  moved from `setupCommand` into `RunCommand` (consolidated to 73-78, comment
  + check + return — the setupErr-propagation pin folds into the same block);
  the pipeline wirePipes/wirePipesFn defensive pins
  (32-34, 34-36, 41-42, 42-42, 44-46) are **removed** — the seam was deleted
  with the `tools.ProcessRunner` port swap (ADR-074), pipe creation dissolving
  into the adapter.
- **See**: `internal/infrastructure/di/container.go:284-286`,
  ...
  `internal/tools/workspace/process_executor.go:73-78`,
...
- **Rationale**: triggering the branch requires filesystem, driver, environment,
  ...
  Architect-acceptance comments at representative sites.
  Re-anchored/re-classified 2026-09 (#1460 T4): the workspace
  process-execution cluster moved behind the injected `tools.ProcessRunner`
  port (ADR-074). The executor Start/Wait/exit-extraction pins and the
  pipeline start/wait/capture pins are **port-mediated** (interim re-anchored
  to the live `RunCommand`/`RunPipeline`/`pipeline` lines; fake-drivable after
  T6 — pins removed in the T6 commit). The RunPipeline arity guard, setup-error
  propagation, and start-error cleanup survive unchanged and are re-anchored;
  the Output/CombinedOutput error branches are re-anchored (the six
  overlapping #1431 pins consolidate to the four live branch ranges). The five
  `proc_posix.go` refs are **removed**: the file relocated verbatim to
  `internal/infrastructure/process/proc_posix.go` (ADR-074 D4/T3, commit
  `c421b24e`) with its Cancel-closure syscall/time touchpoints behind injected
  hooks — deterministic coverage via adapter-internal tests plus the
  real-process suites; pins closed (move disposition, architect's call at ADR
  time). `reader.go` refs re-verified untouched (no criterion weight).
  `shell.go` refs unchanged (the constructor grew a parameter on existing
  lines; zero line drift).
  Closed 2026-09 (#1460 T6): the executor and pipeline interim pins
  (14 + 5 sites) are **removed** — every branch is deterministically drivable
  and covered by the fake-based suites (`process_executor_fault_test.go`,
  `pipeline_fault_test.go`: start-error wrapping, exit-code surfacing incl.
  the −1 signal convention, non-ExitError passthrough, ctx-cancel partial
  output, capture warning paths, Output/CombinedOutput contracts, pipeline
  start-error cleanup with no further starts, arity and spec-assembly guards,
  reverse-loop wait semantics with lastErr from the last command). Shell
  verdicts (zero line drift, so the #1431 ranges are the live ranges):
  12 refs **re-classified and removed** — ExecuteCommand's
  closure/error/timeout branches (259-271, 273-277), PipeCommands'
  closure/error/timeout branches (364-382), runWithFeedback's three
  publications (466-485), and formatTimeoutResult's partial-output section
  (495) are covered by `shell_fault_test.go`'s two-layer fake
  (fake runner → real processExecutor → real shellTool); 2 refs
  **re-anchored/kept** — `shell.go:538-540,540-541` (warnWriter's
  partial-line break: behavioral path alive, not covered by the fake suites,
  ranges unchanged). Reader refs unchanged (untouched; no criterion weight).
- **See**: `internal/tools/workspace/reader.go:310-312,344-345`,
  `internal/tools/workspace/shell.go:538-540,540-541`,
  `internal/infrastructure/mcp/stdio_client.go:123-125`,
...
- **Rationale**: thin pass-throughs that would only re-test the underlying
  ...
  Re-anchored 2026-09 (#1460 T4): the executor LookPath delegation drifted
  496-498 → 458-461 as the setupCommand/constructor deletions shrank the file
  above it.
- **See**: `internal/tools/workspace/process_executor.go:458-461`,
...
- **See**: `internal/infrastructure/telemetry/system_metrics_darwin_cgo.go:38-41,41-44,45-45,67-69,76-78,87-89`,
  `internal/tools/workspace/process_executor.go:331-333,335-337,341-341`

## Test Complexity (ACCEPTED — issue #1444 Gemini mixed-media ordering tests)

### llm/gemini/adapter_test.go — TestToSDKContent_MixedUserTurn_OrdersInlineDataFirst (CC=13)

- **Status**: ACCEPTED (2026-09, issue #1444)
- **Rationale**: Wire-order pin for the mixed-media user-turn normalization (FR-before-InlineData invalidation, #1441): index-pinned assertions over the four parts plus data/ID preservation checks. CC comes from assertion boilerplate across the per-index checks, not branching business logic. Same acceptance class as `TestToSDKSchema_EmptyType_OmitsTypeKey` (CC=19) — assertion boilerplate across a coverage matrix.
- **See**: `internal/infrastructure/llm/gemini/adapter_test.go:290`

### llm/gemini/adapter_test.go — TestToSDKContent_PoisonedPersistedShape_Normalizes (CC=15)

- **Status**: ACCEPTED (2026-09, issue #1444)
- **Rationale**: Normalization pin for the persisted-history shape (`Data: nil` + `AssetID`): per-index order assertions plus the base64 hydration proof (resolver bytes must land in `InlineData.Data`). CC is assertion boilerplate (order checks + hydration checks + resolver switch), not branching business logic. Same acceptance class as `TestGetModelTurn` (CC=16).
- **See**: `internal/infrastructure/llm/gemini/adapter_test.go:326`

### llm/gemini/adapter_test.go — TestToSDKContent_OtherPartsMoveToEnd (CC=15)

- **Status**: ACCEPTED (2026-09, issue #1444)
- **Rationale**: Table-driven discriminator slices pinning the "other"-to-end rule of the three-zone partition; each case's `check` closure performs per-index class and identity assertions. CC is subtest enumeration + assertion boilerplate, not branching business logic. Same acceptance class as `TestToSDKSchema_EmptyType_OmitsTypeKey` (CC=19).
- **See**: `internal/infrastructure/llm/gemini/adapter_test.go:384`

### llm/gemini/mixed_media_roundtrip_test.go — TestSendChat_MixedMediaStoreRoundTrip (CC=15)
...
### process/runner.go — StdoutPipe/StderrPipe errors (structurally unreachable)

- **Status**: ACCEPTED (2026-09, issue #1462)
- **Rationale**: both pipes are created inside `Start` before `cmd.Start` (ADR-074 D2), so `os/exec` StdoutPipe/StderrPipe can only fail when called after `Start` or on a second call — structurally unreachable in correct code. Same acceptance class as the cataloged `mcp/stdio_client.go` StdoutPipe/StdinPipe entry. Lifecycle: originally pinned at `process_executor.go:143-145`/`:147-149`, re-anchored 2026-09 (#1431) to `152-154`/`:156-158`, removed 2026-09 (#1460 T4) when the property dissolved into the relocated adapter, re-established 2026-09 (#1462) at the relocated sites; re-verified against the live source.
- **See**: `internal/infrastructure/process/runner.go:66-70,72-75`
...
## Coverage Gaps (ACCEPTED — 2026-09 ADR-076 callback coverage follow-up, issue #1488)

### cli/chat_command.go — ChatService nil guard in executeInference (defensive guard)

- **Status**: ACCEPTED (2026-09, issue #1488)
- **Rationale**: composition-boundary nil guard on the ADR-076 callback execution path; `newChatCommand` always copies `ChatService` from the DI-composed `context` (ADR-076 A5 injects the notifier/mode-lock ports via `cli.AppDependencies`), so callers never produce nil post-DI. The guard fails fast on programmer error only. Same acceptance class as the defensive nil guards on internal pipeline state (2026-07 Batch Triage).
- **See**: `internal/cli/chat_command.go:253-255`

### cli/chat_command.go — CallbackNotifier nil guard in deliverCallback (defensive guard)

- **Status**: ACCEPTED (2026-09, issue #1488)
- **Rationale**: same composition-boundary rationale as the ChatService guard: the ADR-076 callback notifier port is always injected at the composition root, so the nil branch is unreachable in correct DI composition. Fails fast on programmer error only.
- **See**: `internal/cli/chat_command.go:308-310`

### cli/chat_command.go — ModeLocker nil guard in acquireModeLock (defensive guard)

- **Status**: ACCEPTED (2026-09, issue #1488)
- **Rationale**: same composition-boundary rationale: the ADR-076 D3 mode-lock port is always injected; callback mode cannot reach the lock step without DI providing the locker. Fails fast on programmer error only.
- **See**: `internal/cli/chat_command.go:472-475`

### infrastructure/callback/http_notifier.go — json.Marshal(CallbackPayload) error (structurally unreachable)

- **Status**: ACCEPTED (2026-09, issue #1488)
- **Rationale**: `json.Marshal(payload)` cannot fail: `CallbackPayload` is the ADR-076 D8 closed wire contract of JSON-safe scalar fields (SessionID/Status/Response string, Error *string). The `if err != nil` branch exists only for interface contract compliance. Same acceptance class as the cataloged `json.Marshal` entries (`global_prompt_tracker.go` Append/writeCompactedData, `orchestrator/middleware.go` detectLoop).
- **See**: `internal/infrastructure/callback/http_notifier.go:49-51`

*Last Updated: 2026-09 (ADR-053: get_cost_summary tool, DailyCost metric, and global cost ledger removed — issue #1291; coverage/complexity hygiene: event-type table test, shared markdown renderer, accepted mock stubs; catalog additions: ado/pipeline_crud.go json.Marshal unreachable entry, assertMissingKeysResult (CC=13), TestRecoveryStep_EmptyResponse_RetriesUpToLimit (CC=12), CC drift re-verification for (*indexer).snapshot (14), TestResolveCapabilities (13), TestFixtureIndexer_ConstructAndHarvest (13), design rejection: split internal/agent façade — issue #1299; #1302: emergencySave ghost-response guard entry — engine_phases.go; #1300: #1299 entry criterion renamed di-touch → cross-layer per ADR-056; coverage hygiene: NoOpLogger + BypassConfirmation entries — 2026-08; test-complexity catalog additions: TestFakeToolchainRunner_PresetValues (CC=28) and TestFakeToolchainRunner_ZeroDefaults (CC=38) — issue #1325 toolstest fake; #1327: middleware.go catalog pins re-anchored to :213/:311 (per-turn Turn lifecycle refactor); catalog re-anchor: di/container.go skills.sh error branch (203-206) covered — See narrowed to :197-200; catalog re-anchor: history_test.go pins +1 (T1 import shift); catalog re-anchor: middleware.go detectLoop + session_manager.go TUI entry (line drift from renderPostTUISummary feature); catalog re-anchor: factory_test.go + files_test.go complexity pins (issue #1350 item 4, llm→auth import shift); catalog re-anchor: files_test.go complexity pin 378→379 + auth.go Invalidate no-op stub lines (issue #1358 auth/contract realignment); 2026-09 factory seams triage: coverage pin for llm/factory.go createAuthenticator default-case; 2026-09 catalog hygiene: coverage pins for llm/failover.go ExtractDocument delegation wrapper + llm/provider_health.go getPingEndpoint panic and buildRequest error branch (structurally unreachable); re-anchored NewID (222-224 → 234-236) and Task accessors (105,108,111 → 112,116); coverage test for BuildSuggestionService tracker fallback; 2026-09 #1378: four test-complexity catalog additions (TestToSDKSchema_EmptyType_OmitsTypeKey CC=19, TestToOpenAISchema_EmptyTypeOmitsKey CC=11, TestToOpenAITools_EmptyTypeNested_OmitsEmptyTypeKey CC=16, TestConvertSchema_NestedUnrepresentable_BecomesUntypedAny CC=12) — MCP empty-type schema tests; #1387: buildFileUploadBody multipart + propagation coverage-pin catalog entries (Entry A/B); redact hygiene: normalizeKeyToken dead-branch coverage entry (structurally-unreachable); 2026-09 triage completion: ten coverage entries (mcp marshalInputSchema/pipes/resolveCommand/Close-race, global_prompt_tracker prepare, policy confirmAction call-sites, manager Abs guards, state SetInfo marshal, telemetry summary marshal, toolchain execRunner); re-anchors (sqlite RowsAffected 200/218 → 207-209/224-226, state.go:56 dropped, gh token resolver 66-70 → 66-71); partition-test rows (17); complexity-drift repair: TestBasicAuthTransport 645→646, TestMCPFactory_ConcurrentBuild 683→684, ..._FailingServerSkips 743→744 (import-shift re-anchors), TestStdio_ChildDeathMidSession CC 11→13, TestResolveCommand 10→13 new entry (stdio_client_test.go:25); memory integration: hook.go BeforeTurn/OnPhaseTransition interface-stub entry + TestFirstNonNil/TestAcquireWriteLockUserHomeDirError coverage — partition-test row (1); memory integration round 2: truncateToBytes/AfterTurn-empty-text/acquireWriteLock call-site guard/metadataIDs/lastUserText/joinTextParts/FlushSession edge coverage (harness fix: newTestMemoryHome pre-creates ~/.plur) + flock_unix.go non-EWOULDBLOCK (fault-injection-required) and FlushSession engrams-empty (defensive-guard) catalog entries — partition-test rows (2); #1431: medium-gap triage batch (140 sites across six classes: defensive 53, structurally-unreachable 5, fault-injection 55, interface-stub 16, delegation 2, platform 9) + TurnTrace.AddWarnings / startWalker walk-error coverage fixes + six coverage-pin re-anchors (StdoutPipe 152-154, StderrPipe 156-158, renderMarkdownAsync 669-676, stdio_client ListTools 174-176, Close 251-253, resolveCommand 358) + partition-test rows (6), CC=10 boundary watch list)) ; #1444: five test-complexity catalog additions (Gemini mixed-media ordering tests — TestToSDKContent_MixedUserTurn_OrdersInlineDataFirst CC=13, TestToSDKContent_PoisonedPersistedShape_Normalizes CC=15, TestToSDKContent_OtherPartsMoveToEnd CC=15, TestSendChat_MixedMediaStoreRoundTrip CC=15, validateMixedMediaRoundTripReq CC=37) — partition-test rows (5); #1460 T4: seam-swap catalog batch — #1431 fault-injection entry re-anchored/re-classified (workspace executor/pipeline pins port-mediated with interim re-anchors to the live RunCommand/RunPipeline/pipeline lines, proc_posix 5 pins removed as a file relocation to internal/infrastructure/process with injected-kill-function coverage (commit c421b24e), reader re-verified untouched, shell zero-drift); the two structurally-unreachable StdoutPipe/StderrPipe entries (process_executor.go:152-154/:156-158) **removed** — the property dissolves into internal/infrastructure/process/runner.go where pre-Start pipe creation preserves it (ADR-074 D2); defensive 53-site entry: executor guard re-anchored to RunCommand 73-78, pipeline wirePipes/wirePipesFn pins removed (seam deleted with the port swap); platform-specific entry re-anchored (windows branch 331-341); delegation-wrapper entry re-anchored (LookPath 458-461); TestRunCommand_NonExitErrorWaitPath pin re-verified unshifted (stream_test.go:817, CC=11 unchanged — rename-only edits); partition-test workspace rows updated in the same commit per the Drift Policy coordination rule; #1460 T6: fake-based deterministic closure batch — the 19 executor+pipeline fault-injection interim pins removed (14 executor + 5 pipeline sites, every branch covered by `process_executor_fault_test.go`/`pipeline_fault_test.go`/`shell_fault_test.go`); shell.go per-pin re-triage: 12 refs re-classified and removed (ExecuteCommand/PipeCommands error+timeout branches, runWithFeedback publications, formatTimeoutResult partial-output — two-layer fake coverage), 2 refs re-anchored/kept (warnWriter partial-line break 538-541, alive + uncovered); reader refs kept (untouched); **headline: net catalog reduction ≥24 achieved** (T4's proc_posix move-closures + T6's 19 fake-based closures + 12 shell re-classifications); five new test-complexity entries + partition rows (TestRunCommandFaultPaths CC=26, TestRunPipelineFaultCleanup CC=16, TestPipelineWaitFaultSemantics CC=15, TestShellFaultPipeCommands CC=14, TestShellFaultExecuteCommand CC=12); TestRunCommand_NonExitErrorWaitPath Start-error partition row removed with its pin; TestRunCommand_NonExitErrorWaitPath complexity pin re-anchored 817→818 (T5 comment-hygiene shift, CC re-verified 11); #1460 T8: final sweep — Makefile exclusion truth repair (the dead workspace default_fs.go exclusions removed from verify-tools-infrastructure-import's POSIX/PowerShell branches; adapter-gate header + help aligned to the single sanctioned analysis/default_fs.go path per the ADR-055 amendment), all pins re-verified against the live tree, residue documented (modelith-drift advisory disposition: `toolstest.FakeProcessRunner` flagged unmodeled — accepted, test doubles are deliberately not domain-model entries, mirroring the unmodeled `FakeToolchainRunner` precedent; #1462 B3: execRunner closure pin (toolchain_factory.go, was :154-156 after the #1460 T4 +5 drift) removed — the branch is now covered by TestBuildRegistry_InstallSkillExecutesRealRunnerClosure (pre-cancelled-context install_skill drive); partition row removed in the same commit per the Drift Policy coordination rule; #1462: process-package catalog batch — StdoutPipe/StderrPipe pins re-established at runner.go:66-70/:72-75 (one entry, comma-separated ranges — re-establishment after the #1460 T4 removal, not a re-anchor), proc_posix C1-C3 adjudications recorded PROPOSED (nil-guard defensive, non-ESRCH + ErrProcessDone fault-injection; maintainer approval at PR review), registerSkillsShTools error pin re-anchored 127-129 → 132-134 and re-classified structurally-unreachable (fact-check: RegisterSkillsShTools cannot fail via BuildRegistry) — partition-test rows (5 added, 1 updated) in the same commit per the Drift Policy coordination rule; #1488: ADR-076 callback coverage triage — four coverage-pin entries (cli nil guards ×3, http_notifier marshal) + Part C approvals)
